...
Notice how $T_1$ and $T_2$ are defined as before, and we already have bounds for both of them. 
...
The third inequality holds since $\pi_0$ was initialized as $$\pi_0 = \arg\min_{\pi \in \mathcal{M}_1(A)} F(\pi)$$ so we have that $F(\pi_{k-1}) - F(\pi_0) \ge 0$. 
