...
Note that to calculate $$\pi_k(a\vert s)$$, one does needs to calculate
$$E_k(s,a)=\exp\left( \eta \phi(s,a)^\top \bar \theta_{k-1}\right)$$ and then compute $$\pi_k(a\vert s) = E_k(s,a)/\sum_{a'} E_k(s,a')$$.
