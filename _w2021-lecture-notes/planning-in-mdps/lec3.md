...
comments: true
---
Last time, we discussed the Fundamental Theorem of Dynamic Programming, which then lead to the efficient "value iteration" algorithm for finding the optimal value function.
...
the
 $\tilde{O}(\cdot)$ result holds when $\delta \le 1/e$ is fixed and $\tilde{O}(\cdot)$ hides $\log(2/(1-\gamma))$.
...
Note that the relative error bound is not without problems either: It is possible that for some states $s$, $$v^*(s)-\delta_{\text{rel}} \|v^*\|_\infty$$ is negative, a vacuous guarantee. A reasonable stopping criteria would be top stop when the policy that we read out satisfies
