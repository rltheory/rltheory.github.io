...
We consider fixed horizon local planning in large finite MDPs $(\mathcal{S},\mathcal{A},P,r)$.
...
the action-value functions depend on the number of steps left, or the current stage.
...
For $\delta>0$, we call a local planner **$\delta$-sound for the $H$-step criterion** if for any MDP $M$ and feature map $\phi = (\phi_h)_h$ pair such that the optimal action-value function of $M$ is realizable with the features $\phi$ in the sense that \eqref{eq:qsrealizability} holds, the planner
...
The main result of this lecture as follows:

---
**Theorem (worst-case query-cost is exponential under $q^*$-realizability)**:
For any $d,H$ large enough and any local planner $\mathcal{P}$ that is
...
Whether an example exist with only a few actions but with a more complicated dynamics remains open. With the construction provided here (which is based on a tree dynamics and zero intermediate reward in the tree), this clearly fails, as we will make it clear below.
